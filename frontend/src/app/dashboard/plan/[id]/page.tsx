...
import { getStudyPlan, getPlanStats, updateTaskCompletion } from '@/lib/api';
...
export default function PlanDetailPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (session?.user) {
          ...
          await loadPlanData(sessionToken);
        } else {
  ...
  const loadPlanData = async (authToken: string) => {
    setIsLoading(true);
    setError(null);

    try {
      // Fetch plan with tasks
      const planData = await getStudyPlan(planId, authToken);
      ...
      // Fetch stats
      const statsData = await getPlanStats(planId, authToken);
  ...
  const handleTaskToggle = async (taskId: string, isCompleted: boolean) => {
    try {
      const data = await updateTaskCompletion(planId, taskId, isCompleted, token);
      ...
      if (data.success && data.task) {
        ...
        // Refresh stats
        const statsData = await getPlanStats(planId, token);
        if (statsData.success && statsData.stats) {
          setStats(statsData.stats);
        }
      }
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 py-8 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <ArrowLeft size={16} />
            <span className="text-sm">Back to Study Plans</span>
          </Link>
